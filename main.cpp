...
void testprintTheArray (Env env);
void testNodeListCopy();
...
/* Nodelist cotains startnode and goal node, I used solution->getLenth() - 2 to remove start and goal node from the loop count. */
for (int i = 0; i < solution->getLength() - 2; i++)
{
     if (solution->getNode(i+1)->getCol() > solution->getNode(i)->getCol() && 
     env[solution->getNode(i+1)->getRow()][solution->getNode(i+1)->getCol() != SYMBOL_GOAL || SYMBOL_WALL]){
...
void testNodeListCopy (){
    ...
    NodeList* nodeListPtr = new NodeList();
    // Making a new test node with a distance of 3
    Node* testNode = new Node(1,1,3);

    nodeListPtr->addElement(testNode);

    // printing the array length
    std::cout << nodeListPtr->getLength() << '\n';

    // copying the nodelistPtr
    NodeList* copyNodeList = new NodeList(*nodeListPtr);
    std::cout << copyNodeList->getLength() << '\n';
    std::cout << copyNodeList->getNode(0)->getDistanceTraveled() << "\n";
